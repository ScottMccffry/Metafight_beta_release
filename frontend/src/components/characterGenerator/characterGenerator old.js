...
const CharacterGenerator = () => {
  ...
  const MFTaddress ='0x075ddf674AeB92Fb5221DBB2393A970D731a4bc0' 
...
{/*
    ...
    const createCombinedImage = async () => {
      ...
      if (bodyType) {
        const fighterImg = await loadImage(`../../assets/character/${bodyType}.png`);
        ctx.drawImage(fighterImg, 0, 0);
      }
      
      if (weaponsType) {
        const weaponImg = await loadImage(`../../assets/weapons/${weaponsType}.png`);
        ctx.drawImage(weaponImg, 0, 0);
      }
